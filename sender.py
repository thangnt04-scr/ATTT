...
def sender_thread(file_path, status_queue, host, use_cloud):
    ...
    try:
        ...
        for i in range(num_expected_parts):
            ...
            parts.append(file_data[current_pos : current_pos + part_size_actual])
        ...
        if not use_cloud:
            for i, part_content in enumerate(parts):
                ...
                # >>> BẮT ĐẦU SỬA LỖI <<<
                # Ký số cho phần hiện tại để đảm bảo mỗi phần có chữ ký riêng
                part_signature = private_key.sign(
                    encrypted_part, 
                    padding.PKCS1v15(), 
                    hashes.SHA512()
                )
                
                # Gói tin JSON, đảm bảo sử dụng đúng chữ ký vừa tạo
                packet = {
                    "iv": base64.b64encode(iv).decode(),
                    "cipher": base64.b64encode(encrypted_part).decode(),
                    "hash": hash_value.hex(),
                    "sig": base64.b64encode(part_signature).decode()
                }
                # >>> KẾT THÚC SỬA LỖI <<<
